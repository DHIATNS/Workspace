...
function getComments($identifier)
{
 	...
 	$database = dbConnect();
    $statement = $database->prepare(
        "SELECT comments.id, author, comment, DATE_FORMAT(comment_date, '%d/%m/%Y à %Hh%imin%ss')AS french_creation_date , USER.name FROM comments, USER WHERE USER.id=comments.author
